...
func filterThinkTags(msgs []api.Message, m *Model) []api.Message {
	if m.Config.ModelFamily == "exaone" || m.Config.ModelFamily == "qwen3" || model.ParseName(m.Name).Model == "deepseek-r1" {
		finalUserIndex := -1
		...
		thinkOpeningTag, thinkClosingTag := "", ""
		defaultOpeningTag := "<think>"
		defaultClosingTag := "</think>"
		if m.Config.ModelFamily == "exaone" {
			thinkOpeningTag = "<thought>"
			thinkClosingTag = "</thought>"
		} else {
			thinkOpeningTag = defaultOpeningTag
			thinkClosingTag = defaultClosingTag
		}
		for i, msg := range msgs {
			if msg.Role == "assistant" && i < finalUserIndex {
				...
				thinkingState := &thinkingParser{
					openingTag: thinkOpeningTag,
					closingTag: thinkClosingTag,
				}
